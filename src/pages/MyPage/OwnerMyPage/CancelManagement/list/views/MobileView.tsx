...
export default function MobileView() {
  ...
  /** 취소 요청 목록 불러오기 */
  useEffect(() => {
    if (!owner) return; // owner가 없으면 호출하지 않음
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto">
        {/* 상단 여백 + 컨텐츠 래퍼 */}
        <div className="mt-[90px] flex flex-col gap-6 px-5">
          {/* 상단 라벨 */}
          <div className="flex h-[21px] w-full flex-row items-center justify-between">
              ...
              sortedItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="w-full cursor-pointer border-b border-[#F3F4F5] bg-white py-3"
                  >
                    <div className="flex h-[101px] w-full flex-row items-center justify-between">
                      <div className="flex h-[69px] flex-1 flex-col justify-between gap-1 text-left">
                        <div className="flex flex-col">
                          <span className="text-[14px] font-semibold text-[#000000]">
                            {item.shopName}
                          </span>
                          <span className="mt-[2px] line-clamp-1 text-[16px] text-[#000000]">
                            {item.productName}
                          </span>
                        </div>
                      ...
                      <div
                        className={`ml-3 flex h-[33px] min-w-[48px] items-center justify-center rounded-[20px] px-3 py-[6px] ${badge.className}`}
                      >

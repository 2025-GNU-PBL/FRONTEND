...
const MobileView = () => {
  ...
  if (error) {
    return (
      <div className="mobile-cart-page-container error-message">{error}</div>
    );
  }
      ...
      <div className="mobile-cart-page-container">
        <div className="mobile-cart-header">
          <Icon
            ...
            className="mobile-header-back-arrow"
            onClick={() => navigate(-1)}
          />{" "}
          {/* 뒤로가기 기능 추가 */}
        ...
        <div className="mobile-empty-cart-view">
          ...
          <p className="mobile-empty-cart-text">
            장바구니에 담긴 상품이 없어요.
          </p>
          <button
            className="mobile-go-to-products-button"
            onClick={() => navigate("/")}
          >
            상품 보러가기
          </button>
  ...
  return (
    <div className="mobile-cart-page-container">
      ...
      <div className="mobile-separator-gray-8px"></div>{" "}
      {/* Separator below header */}
      {/* Select All / Delete Selected */}
      <div className="mobile-select-delete-section">
        <div className="mobile-select-all-group">
          <button
            className={`mobile-product-item-checkbox ${
              isAllChecked
                ? "mobile-product-item-checkbox-checked"
                : "mobile-product-item-checkbox-unchecked"
            }`}
        ...
        <span
          className="mobile-delete-selected-text"
      ...
      {cartData.items.map((item) => (
        <div className="mobile-product-item-wrapper" key={item.cartItemId}>
          <button
            ...
            onClick={() =>
              toggleItemCheckbox(item.cartItemId, item.quantity, item.selected)
            }
          ...
          <img
            ...
            className="mobile-product-item-image"
          />
          <div className="mobile-product-details">
            <p className="mobile-product-item-shop-name">{item.bzName}</p>{" "}
            {/* TODO: 실제 상점 이름으로 대체 */}
            <p className="mobile-product-item-description">
              {item.productName}
              <br />
              (수량: {item.quantity})
            </p>
            <div className="mobile-product-price-section">
              <p className="mobile-product-item-original-price">
                {(item.price * item.quantity).toLocaleString()}원
              </p>
              <p className="mobile-product-item-discounted-price">
                {(item.price * item.quantity).toLocaleString()}원
              </p>{" "}
              {/* TODO: 할인 금액 계산 */}
            </div>
            <div className="mobile-product-item-quantity-control">
              <Icon
                icon="mynaui:minus"
                className="mobile-quantity-minus-icon"
                onClick={() =>
                  decreaseQuantity(
                    item.cartItemId,
                    item.quantity,
              ...
              <Icon
                icon="mynaui:plus"
                className="mobile-quantity-plus-icon"
                onClick={() =>
                  increaseQuantity(
                    item.cartItemId,
                    item.quantity,
      ...
      <div className="mobile-summary-section">
        <div className="mobile-summary-item">
          ...
          <p className="mobile-summary-item-value">
            {cartData.totalProductAmount.toLocaleString()}원
          </p>
        </div>
        <div className="mobile-summary-item">
          <p className="mobile-summary-item-label">총 할인금액</p>
          <p className="mobile-summary-item-value">
            {cartData.totalDiscountAmount.toLocaleString()}원
          </p>
        </div>
        <div className="mobile-summary-item">
          <p className="mobile-summary-item-payment-amount">결제금액</p>
          <p className="mobile-payment-value">
            {cartData.paymentAmount.toLocaleString()}원
          </p>
        </div>
      </div>
      {/* Purchase Bar */}
      ...
      {isPopupOpen && (
        <div className="mobile-dimmed-overlay" onClick={togglePopup}>
          <div
            className={`mobile-popup-container ${isPopupOpen ? "" : "hidden"}`}
          ...
          >
            <div className="mobile-popup-header">
              <div className="mobile-popup-product-info">
                <img
                  src={
                    cartData.items[0]?.thumbnailUrl || "/images/placeholder.png"
                  }
                  alt="Product"
                  className="mobile-popup-product-image"
                />
                <div className="mobile-popup-product-text-group">
                  <p className="mobile-popup-product-shop-name">
                    제이바이로이스타
                  </p>
                  <p className="mobile-popup-product-description">
                    {cartData.items[0]?.productName}
                    <br />
                  </p>
                </div>
              </div>
              <Icon
                ...
                className="mobile-popup-close-icon"
                onClick={togglePopup}
              />
            </div>
            ...
            <div className="mobile-popup-summary-section">
              ...
              <div className="mobile-popup-expected-payment-row">
                <p className="mobile-popup-expected-payment-label">
                  예상 결제금액
                </p>
                <p className="mobile-popup-expected-payment-value">
                  {cartData.paymentAmount.toLocaleString()}원
                </p>
              </div>
            </div>
            ...
            <div className="mobile-popup-buttons-container">
              ...
              <button
                className="mobile-popup-purchase-button"

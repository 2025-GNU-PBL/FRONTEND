...
type DressTag =
  ...
  | "DOMESTIC"
  | "IMPORTED"
  | "DOMESTIC_AND_IMPORTED";

const TAG_LABEL: Record<DressTag, string> = {
...
const TAG_GROUPS: { title: string; items: DressTag[] }[] = [
  {
    title: "행사",
...
const WebView: React.FC = () => {
  ...
  /** ✅ 태그: 패널 내 선택값과 실제 적용값 분리 (드레스 태그) */
  const [selectedTags, setSelectedTags] = useState<Set<DressTag>>(new Set());
  const [appliedTags, setAppliedTags] = useState<Set<DressTag>>(new Set());
  ...
  const fetchMore = useCallback(
    async (page: number) => {
      ...
      try {
        const { data } = await api.get<PagedResponse>("/api/v1/dress/filter", {
          params: {
            pageNumber: page,
            pageSize,
            region: regionParam,
  ...
  // ✅ 태그: 패널 내 임시 선택 토글
  const toggleTag = useCallback((tag: DressTag) => {
  ...
  const clearAppliedTag = useCallback((tag: DressTag) => {
    setAppliedTags((prev) => {
      const next = new Set(prev);
      next.delete(tag);
      return next;
  ...
  const activeFilterCount = appliedFilterCount(appliedTags);
...
const appliedFilterCount = (set: Set<DressTag>) => set.size;

...
const MobileView: React.FC = () => {
  ...
  const [selectedTags, setSelectedTags] = useState<Set<DressTag>>(new Set()); // 시트 내 임시 선택
  const [appliedTags, setAppliedTags] = useState<Set<DressTag>>(new Set()); // 실제 API에 쓰는 태그
    ...
    const fetchPreview = async () => {
      try {
        ...
        if (
          ...
          console.error("필터 미리보기 개수 조회 실패:", err);
          setPreviewCount(null);
        }
      } finally {
